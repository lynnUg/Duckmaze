...
class SoundManager:
	...
	def __init__( self, config, music_filename = None ):
		...
		if music_filename != None:
			self.music_filename = os.path.join( config.music_dir,
				music_filename )
			#print self.music_filename
		else:
			self.music_filename = ""

		self.sounds_dir = os.path.join( config.install_dir, "sounds" )
        #self.music_dir=os.path.join( config.install_dir, "music" )
		self.setup( None )

	def add_sample_group( self, groupname, filenames ):
		self.config.sound_effects_on=1
		if self.sound_inited:
			self.sample_filenames[groupname] = []
			for fn in filenames:
				self.sample_filenames[groupname].append(os.path.join( self.sounds_dir, fn + ".ogg" ))
	...
	def play_sample( self, groupname ):
		if self.sound_inited:
			print "in play soound",self.sound_effects_on
			if self.sound_effects_on and self.volume > 0:
				#print self.sample_groups
				if self.current_sample:
					print "there's a sample"
					group = self.sample_groups[self.current_sample]
					group[ 0 ].stop()
					group=self.sample_groups[groupname]
					group[0].play(-1)
					self.current_sample=groupname
	...
	def music_start( self ):
		print self.sound_inited,self.music_playing
		if self.sound_inited and not self.music_playing:
			print "in music start"
			if self.music_sample != None:
				print "playing song"
